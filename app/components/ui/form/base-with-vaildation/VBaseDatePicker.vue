  ...
  <div class="w-full">
    ...
    >
      ...
      <span class="ds-adorn ds-adorn--end" :class="{ bordered: borderedEnd }">
        <slot name="end"><Calendar2Line /></slot>
      </span>
    </div>
...
const baseId = computed(() => props.id ?? `${props.name ?? 'ui-date'}-${inst?.uid ?? '0'}`)
...
const computedConfig = computed(() => ({ ...baseConfig.value, ...props.config }))
