<template>
  <!-- Trigger -->
  <Button
    :variant="triggerVariant"
    :color="current.triggerColor"
    @click="open = true"
    v-bind="triggerProps"
  >
    <slot name="trigger">
      {{ BtnText }}
    </slot>
  </Button>

  <!-- Modal -->
  <BaseModal
    v-model="open"
    :rounded="true"
    :hideDefaultClose="hideCornerClose"
    :size="size"
    :fullScreenBackdrop="true"
    :closeOnBackdrop="closeOnBackdrop"
    :closeOnEsc="closeOnEsc"
  >
    <template #body>
      <div class="text-center">
        <!-- Icon blob -->
        <div class="relative z-1 mb-7 flex items-center justify-center">
          <slot name="icon">
            <!-- Outer blob — tinted by status -->
            <svg
              :class="current.blobClass"
              width="90"
              height="90"
              viewBox="0 0 90 90"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M34.364 6.85053C38.6205 -2.28351 51.3795 -2.28351 55.636 6.85053C58.0129 11.951 63.5594 14.6722 68.9556 13.3853C78.6192 11.0807 86.5743 21.2433 82.2185 30.3287C79.7862 35.402 81.1561 41.5165 85.5082 45.0122C93.3019 51.2725 90.4628 63.9451 80.7747 66.1403C75.3648 67.3661 71.5265 72.2695 71.5572 77.9156C71.6123 88.0265 60.1169 93.6664 52.3918 87.3184C48.0781 83.7737 41.9219 83.7737 37.6082 87.3184C29.8831 93.6664 18.3877 88.0266 18.4428 77.9156C18.4735 72.2695 14.6352 67.3661 9.22531 66.1403C-0.462787 63.9451 -3.30193 51.2725 4.49185 45.0122C8.84391 41.5165 10.2138 35.402 7.78151 30.3287C3.42572 21.2433 11.3808 11.0807 21.0444 13.3853C26.4406 14.6722 31.9871 11.951 34.364 6.85053Z"
              />
            </svg>

            <!-- Inner glyph (per status) -->
            <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              <!-- success = check, info = i, warning = !, danger = x -->
              <component :is="current.icon" />
            </span>
          </slot>
        </div>

        <!-- Title -->
        <h4 class="sm:text-title-sm mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
          <slot name="title">{{ titleText }}</slot>
        </h4>

        <!-- Description -->
        <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
          <slot name="description">
            {{ description }}
          </slot>
        </p>

        <!-- Actions -->
        <div class="mt-7 flex w-full items-center justify-center gap-3">
          <slot name="actions" :close="close" :confirm="confirm">
            <!-- Default: single primary action -->
            <Button :variant="'solid'" :color="current.buttonColor" @click="close">
              {{ primaryText }}
            </Button>
          </slot>
        </div>
      </div>
    </template>
  </BaseModal>
</template>

<script setup lang="ts">
import { computed, ref } from 'vue'
import Button from '@/components/ui/Button.vue'

type Status = 'success' | 'info' | 'warning' | 'danger'
type ModalSize = 'sm' | 'md' | 'lg' | 'xl' | 'full'

interface Props {
  /** visual + copy preset */
  status?: Status
  /** trigger button text (default slot can override) */
  BtnText?: string
  /** modal title (slot can override) */
  titleText?: string
  /** modal description (slot can override) */
  description?: string
  /** primary action text (slot:actions can override completely) */
  primaryText?: string

  /** base modal options */
  size?: ModalSize
  closeOnBackdrop?: boolean
  closeOnEsc?: boolean
  hideCornerClose?: boolean

  /** trigger button styling & passthrough */
  triggerVariant?: 'solid' | 'outline' | 'ghost' | 'link'
  triggerProps?: Record<string, any>
}

const props = withDefaults(defineProps<Props>(), {
  status: 'success',
  BtnText: 'Success Alert',
  titleText: undefined,
  description: undefined,
  primaryText: 'Okay, Got It',
  size: 'sm',
  closeOnBackdrop: true,
  closeOnEsc: true,
  hideCornerClose: true,
  triggerVariant: 'solid',
  triggerProps: () => ({})
})

const emit = defineEmits<{
  (e: 'open'): void
  (e: 'close'): void
  (e: 'confirm'): void
}>()

const open = ref(false)

/** Map DS classes & icons per status */
const current = computed(() => {
  const map = {
    success: {
      blobClass: 'fill-success-50 dark:fill-success-500/15',
      icon: SuccessIcon,
      buttonColor: 'success' as const,
      defaultTitle: 'Well Done!',
      triggerColor: 'success'
    },
    info: {
      blobClass: 'fill-blue-light-50 dark:fill-blue-light-500/15',
      icon: InfoIcon,
      buttonColor: 'info' as const,
      defaultTitle: 'Heads up',
      triggerColor: 'info'
    },
    warning: {
      blobClass: 'fill-warning-50 dark:fill-warning-500/15',
      icon: WarningIcon,
      buttonColor: 'warning' as const,
      defaultTitle: 'Attention',
      triggerColor: 'warning'
    },
    danger: {
      blobClass: 'fill-error-50 dark:fill-error-500/15',
      icon: DangerIcon,
      buttonColor: 'error' as const,
      defaultTitle: 'Something went wrong',
      triggerColor: 'error'
    }
  } as const

  return map[props.status as keyof typeof map] || map.success
})

/** Derived copy with sensible defaults */
const titleText = computed(() => props.titleText ?? current.value.defaultTitle)
const description = computed(
  () =>
    props.description ??
    'Lorem ipsum dolor sit amet consectetur. Feugiat ipsum libero tempor felis risus nisi non. Quisque eu ut tempor curabitur.'
)

/** Actions */
function close() {
  open.value = false
  emit('close')
}
function confirm() {
  emit('confirm')
  close()
}

/** Side effects */
watch(open, (val) => (val ? emit('open') : null))
</script>

<!-- status glyphs (token-colored) -->
<script lang="ts">
import { defineComponent, h } from 'vue'

export const SuccessIcon = defineComponent({
  name: 'SuccessIcon',
  setup() {
    return () =>
      h(
        'svg',
        {
          class: 'fill-success-600 dark:fill-success-500',
          width: 38,
          height: 38,
          viewBox: '0 0 38 38'
        },
        [
          h('path', {
            'fill-rule': 'evenodd',
            'clip-rule': 'evenodd',
            d: 'M5.9375 19.0004C5.9375 11.7854 11.7864 5.93652 19.0014 5.93652C26.2164 5.93652 32.0653 11.7854 32.0653 19.0004C32.0653 26.2154 26.2164 32.0643 19.0014 32.0643C11.7864 32.0643 5.9375 26.2154 5.9375 19.0004ZM19.0014 2.93652C10.1296 2.93652 2.9375 10.1286 2.9375 19.0004C2.9375 27.8723 10.1296 35.0643 19.0014 35.0643C27.8733 35.0643 35.0653 27.8723 35.0653 19.0004C35.0653 10.1286 27.8733 2.93652 19.0014 2.93652ZM24.7855 17.0575C25.3713 16.4717 25.3713 15.522 24.7855 14.9362C24.1997 14.3504 23.25 14.3504 22.6642 14.9362L17.7177 19.8827L15.3387 17.5037C14.7529 16.9179 13.8031 16.9179 13.2173 17.5037C12.6316 18.0894 12.6316 19.0392 13.2173 19.625L16.657 23.0647C16.9383 23.346 17.3199 23.504 17.7177 23.504C18.1155 23.504 18.4971 23.346 18.7784 23.0647L24.7855 17.0575Z'
          })
        ]
      )
  }
})

export const InfoIcon = defineComponent({
  name: 'InfoIcon',
  setup() {
    return () =>
      h(
        'svg',
        {
          class: 'fill-blue-light-600 dark:fill-blue-light-500',
          width: 38,
          height: 38,
          viewBox: '0 0 24 24'
        },
        [
          h('path', {
            d: 'M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm0 4.5a1.25 1.25 0 1 1 0 2.5 1.25 1.25 0 0 1 0-2.5ZM10.75 10.5h2.5v7h-2.5v-7Z'
          })
        ]
      )
  }
})

export const WarningIcon = defineComponent({
  name: 'WarningIcon',
  setup() {
    return () =>
      h(
        'svg',
        {
          class: 'fill-warning-600 dark:fill-warning-500',
          width: 38,
          height: 38,
          viewBox: '0 0 24 24'
        },
        [
          h('path', {
            d: 'M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0ZM12 8.5a1 1 0 0 1 1 1v3.5a1 1 0 1 1-2 0V9.5a1 1 0 0 1 1-1Zm0 8.75a1.25 1.25 0 1 1 0-2.5 1.25 1.25 0 0 1 0 2.5Z'
          })
        ]
      )
  }
})

export const DangerIcon = defineComponent({
  name: 'DangerIcon',
  setup() {
    return () =>
      h(
        'svg',
        {
          class: 'fill-error-600 dark:fill-error-500',
          width: 38,
          height: 38,
          viewBox: '0 0 24 24'
        },
        [
          h('path', {
            d: 'M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm3.54 5.46a1.25 1.25 0 0 1 0 1.77L13.77 11l1.77 1.77a1.25 1.25 0 0 1-1.77 1.77L12 12.77l-1.77 1.77a1.25 1.25 0 1 1-1.77-1.77L10.23 11 8.46 9.23a1.25 1.25 0 1 1 1.77-1.77L12 9.23l1.77-1.77a1.25 1.25 0 0 1 1.77 0Z'
          })
        ]
      )
  }
})
</script>
