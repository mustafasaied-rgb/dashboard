<!-- BaseCopyInput.vue -->
<template>
  <BaseInput
    :modelValue="modelValue"
    @update:modelValue="$emit('update:modelValue', $event)"
    :isBordered="true"
    v-bind="$attrs"
  >
    <template #end>
      <button
        type="button"
        class="inline-flex items-center gap-1 text-sm font-medium"
        :disabled="isDisabled"
        @click="copy"
        aria-label="Copy value"
      >
        <svg width="18" height="18" viewBox="0 0 20 20" class="fill-current" aria-hidden="true">
          <path
            fill-rule="evenodd" clip-rule="evenodd"
            d="M6.58822 4.58398C6.58822 4.30784 6.81207 4.08398 7.08822 4.08398H15.4154C15.6915 4.08398 15.9154 4.30784 15.9154 4.58398V12.9128C15.9154 13.189 15.6916 13.4128 15.4154 13.4128H7.08821C6.81207 13.4128 6.58822 13.189 6.58822 12.9128V4.58398ZM7.08822 2.58398C5.98365 2.58398 5.08822 3.47942 5.08822 4.58398V5.09416H4.58496C3.48039 5.09416 2.58496 5.98959 2.58496 7.09416V15.4161C2.58496 16.5207 3.48039 17.4161 4.58496 17.4161H12.9069C14.0115 17.4161 14.9069 16.5207 14.9069 15.4161L14.9069 14.9128H15.4154C16.52 14.9128 17.4154 14.0174 17.4154 12.9128L17.4154 4.58398C17.4154 3.47941 16.52 2.58398 15.4154 2.58398H7.08822ZM13.4069 14.9128H7.08821C5.98364 14.9128 5.08822 14.0174 5.08822 12.9128V6.59416H4.58496C4.30882 6.59416 4.08496 6.81801 4.08496 7.09416V15.4161C4.08496 15.6922 4.30882 15.9161 4.58496 15.9161H12.9069C13.183 15.9161 13.4069 15.6922 13.4069 15.4161L13.4069 14.9128Z"
          />
        </svg>
        <span>{{ copyText }}</span>
      </button>
    </template>
  </BaseInput>
</template>

<script setup lang="ts">
import { computed, ref } from 'vue'

const props = withDefaults(
  defineProps<{
    modelValue?: string | number | null
    disabled?: boolean
  }>(),
  { disabled: false }
)

const emit = defineEmits<{
  (e: 'update:modelValue', v: any): void
}>()

const copyText = ref('Copy')

const isDisabled = computed(() => {
  const hasValue =
    props.modelValue !== null &&
    props.modelValue !== undefined &&
    String(props.modelValue).length > 0
  return props.disabled || !hasValue
})

const copy = async () => {
  try {
    await navigator.clipboard.writeText(String(props.modelValue ?? ''))
    copyText.value = 'Copied!'
  } catch {
    copyText.value = 'Failed'
  } finally {
    window.setTimeout(() => (copyText.value = 'Copy'), 1500)
  }
}
</script>
