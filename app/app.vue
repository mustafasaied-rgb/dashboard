  ...
  <ThemeProvider>
    <NuxtLayout>
      <Toaster
        ...
        rich-colors
        :toastOptions="{
          class: locale == 'ar' ? 'font-arabic' : 'font-outfit',
          descriptionClass: locale == 'ar' ? 'font-arabic' : 'font-outfit'
        }"
      />
      <NuxtPage />
    </NuxtLayout>
...
const staticMetaData = computed(() => ({
  title: settings?.[`meta_title_${locale.value}`],
  description: settings?.[`meta_description_${locale.value}`],
  type: 'website',
  image: settings?.favicon || `${config?.public?.SITE_URL}/favicon.ico`,
  keywords: settings?.[`keywords_${locale.value}`],
  author: 'Dashboard'
}))
